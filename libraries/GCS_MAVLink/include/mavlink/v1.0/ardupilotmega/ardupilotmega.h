...
typedef enum MAV_CMD
{
	...
	MAV_CMD_NAV_LOITER_WITH_PARAMS=31, /* Loiter at the specified Latittude, Longitude and Altitude.  If Lat=Lon=0, then loiter at the current position.  If the speed parameter is non-zero, tell the autopilot to attempt to loiter at the specified speed.  Don't consider the navigation command complete until the altitude speed have been reached within the tolerances specified AND the this has been true for at least the specified number of circuits (Min circuits parameter). |Speed (m/s)| Speed tolerance (m/s)| Altitude tolerance (m)| Min circuits| Longitude| Latitude| Altitude|  */
